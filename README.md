...
**Currently implemented.** 28 SVG card endpoints (`/api/*`), 17 built-in themes plus gist-hosted custom palettes via `theme_url=`, five bundled variable fonts, `/api/stack` composition with namespaced child IDs, a live playground at [profilekit.vercel.app](https://profilekit.vercel.app), and the [`profilekit-mcp`](packages/mcp/) package. Two deployment paths: **Vercel functions** (primary, `api/[endpoint].js`) and an **optional self-hosted Docker** image (`Dockerfile` + `server.js`) running the same handlers. Zero runtime dependencies in the rendering service, 30-minute CDN cache on the hosted instance.
...
ProfileKit is a monorepo with independently deployable surfaces:

```text
api/              Vercel adapter
src/              SVG renderers, endpoints, and shared query/theme logic
public/           Browser studio
server.js         Local and Docker HTTP adapter
packages/mcp/     Published profilekit-mcp stdio package
```

The web service remains dependency-free at runtime. The MCP package keeps its
own npm package identity and release cadence, but its offline catalog is
generated from `src/endpoints/catalog.js` so the two surfaces cannot silently
drift.
...
| MCP integration | First-class — [`profilekit-mcp`](packages/mcp/) lets Claude / Cursor / Codex CLI build cards as a tool call | None | None |
...
- `https://gist.githubusercontent.com` is currently the only allowlisted host. Other hosts are rejected by the theme loader; the card falls back to the default `dark` palette and carries an `X-Theme-Error` header.
...
docker run --rm -p 3000:3000 \
  -e GITHUB_TOKEN=REPLACE_WITH_GITHUB_TOKEN \
  profilekit:local
...
- **Now** — 28 card endpoints, 17 themes, playground at [profilekit.vercel.app](https://profilekit.vercel.app), MCP server in [`packages/mcp`](packages/mcp/), curated starting styles in the editor.
