const { fetchActivity } = require("../src/fetchers/activity");
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const events = await fetchActivity(username, token);
    if (events.length === 0) {
      res.setHeader("Cache-Control", errorCacheHeaders());
      return res.send(renderError("No recent activity found", colors));
    }

    const svg = renderActivityCard(events, { colors, hideBorder, hideTitle, title });
