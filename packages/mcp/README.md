...
**MCP server for [ProfileKit](https://github.com/starter-series/ProfileKit).** Build GitHub profile SVG cards through conversation — from Claude Code, Codex CLI, ChatGPT Apps, or any other MCP-capable agent. The package source lives in `packages/mcp` inside the ProfileKit monorepo while retaining an independent npm release.
...
From a ProfileKit checkout, verify the package from the monorepo root:
...
### Self-hosted ProfileKit

The hosted API is the default. To point the MCP server at a local or Docker
deployment, provide the API base URL:

```bash
PROFILEKIT_BASE_URL=http://127.0.0.1:3000/api profilekit-mcp
```

The catalog is read from `${PROFILEKIT_BASE_URL}/catalog`. Set
`PROFILEKIT_CATALOG_URL` only when the catalog must come from a different URL.
