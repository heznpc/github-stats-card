import {
  ...
  getProfileKitBaseUrl,
} from "./config.js";

// Kept for API compatibility. buildCardUrl resolves the environment-backed
// base dynamically so one MCP installation can target a self-hosted service.
export const PROFILEKIT_BASE = DEFAULT_PROFILEKIT_BASE_URL;
...
export function buildCardUrl(
  ...
  const base = getProfileKitBaseUrl();
  return queryString
    ? `${base}/${segment}?${queryString}`
    : `${base}/${segment}`;
}
